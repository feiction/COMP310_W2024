...
int interpreter(char *command_args[], int args_size, char *cwd) {
    ...
    if (strcmp(command_args[0], "help") == 0) { // help
        ...
        // iterate through the user input, and append each of the VAR's to the
        // end of the buffer initialize the string buffer, the size is 505 since
        // there are at most 5 tokens, and each of them is shorter than 100
        // characters there are also spaces between the VAR's, so we add 5 extra
        // slots
    ...
    // FS
        ...
        if (bytes_written == -1) {
            ...
            printf(
                "Warning: could only write %d out of %d bytes (reached end of "
                "file)\n",
                bytes_written, size);
        }
        return 0;
