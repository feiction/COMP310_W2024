...
bool dir_create(block_sector_t sector, size_t entry_cnt) {
    ...
    // The first (offset 0) dir entry is for parent directory; do
    // self-referencing Actual parent directory will be set on execution of
    // dir_add()
...
bool dir_lookup(const struct dir *dir, const char *name, struct inode **inode) {
    ...
    if (strcmp(name, ".") == 0) {
        ...
        *inode = inode_reopen(dir->inode);
        ...
        // parent directory : the information is stored at the first (0-pos)
        // entry.
